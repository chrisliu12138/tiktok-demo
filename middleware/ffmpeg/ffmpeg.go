...
func InitSSH() {
	...
	if "password" == "password" {
		SSHconfig.Auth = []ssh.AuthMethod{ssh.Password("neil66188.")}
	}
	//dial 获取ssh client
	addr := fmt.Sprintf("%s:%d", "192.168.111.132", 22)
...
// 维持长链接
func keepAlive() {
	time.Sleep(time.Duration(10*60) * time.Second)
