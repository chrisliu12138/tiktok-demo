...
type Result struct {
	...
	Name          string    `gorm:"column:name"`           //user
	FollowCount   uint      `gorm:"column:follow_count"`   //user
	FollowerCount uint      `gorm:"column:follower_count"` //user
	IsFollow      uint      `gorm:"column:bool"`           //user
	CreatedAt     time.Time `gorm:"column:create_time"`    //video
}
