...
func FollowList(userId int64) ([]dao.User, error) {
	...
	n, err := redis.RdbFollow.SCard(redis.Ctx, userIdStr).Result()
	if err != nil{
		return nil, err
	}
	//redis中存在记录
	if n > 0{
		followUsers, err := redis.RdbFollow.SMembers(redis.Ctx, userIdStr).Result()
		if err != nil{
			return nil, err
		}
	...
	toUsers := make([]int, 0)
	//从mysql中获取
	if err := dao.Db.Raw("select to_user_id from follow where user_id = ? and cancel = 0", userIdStr).Scan(&toUsers).Error; err != nil{
		return nil, err
	}
...
func FollowerList(userId int64) ([]dao.User, error) {
	//查询redis
	userIdStr := strconv.FormatInt(userId, 10)
	n, err := redis.RdbFollower.Exists(redis.Ctx, userIdStr).Result()
	if err != nil{
		return nil, err
	}
	//redis中存在记录
	if n > 0{
		followerUsers, err := redis.RdbFollower.SMembers(redis.Ctx, userIdStr).Result()
		if err != nil{
			return nil, err
		}
	...
	users := make([]int, 0)
	//从mysql中获取
	if err := dao.Db.Raw("select user_id from follow where to_user_id = ?", userIdStr).Scan(&users).Error; err != nil{
		return nil, err
	}
...
func queryList(userId int64, list []string, follow bool) ([]dao.User, error){
	...
	for _, user := range list{
		...
		if err := dao.Db.Raw("select id, name, follow_count, follower_count from users where id = ?", user).Scan(&tmpUser).Error; err != nil{
			return nil, err
		}

		if follow{
			if err := dao.Db.Raw("select user_id from follow where user_id = ? and to_user_id = ? and cancel = 0", userId, user).Error; err == nil{
				tmpUser.IsFollow = true
			}else if gorm.IsRecordNotFoundError(err){
				log.Println("mysql查询错误: ", err)
			}
		}else{
			if err := dao.Db.Raw("select user_id from follow where user_id = ? and to_user_id = ? and cancel = 0", user, userId).Error; gorm.IsRecordNotFoundError(err){
				tmpUser.IsFollow = true
			}
		}
...
func GetFollowCnt(userId int64) (followCnt int64, err error){
	//查询redis
	userIdStr := strconv.FormatInt(userId, 10)
	followCnt, err = redis.RdbFollow.SCard(redis.Ctx, userIdStr).Result()
	if err != nil{
...
func GetFollowerCnt(userId int64) (followCnt int64, err error){
	...
	toUsers := make([]int, 0)
	//从mysql中获取
	if err := dao.Db.Raw("select user_id from follow where to_user_id = ? and cancel = 0", userIdStr).Scan(&toUsers).Error; err != nil{
		log.Println("查询mysql出错", err)
		return followCnt, err
	}
...
func addFollowToRedis(userId int64, toUsers []int){
	...
	for i := 0; i < len(toUsers); i++ {
		...
		redis.RdbFollow.SRem(redis.Ctx, userIdStr, toUserIdStr)
